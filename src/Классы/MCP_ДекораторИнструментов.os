...
Процедура ПодготовитьИмяОписаниеИнструмента(ПостроительДекоратора, ДанныеЖелудя, Аннотация)
	Если Не ЕстьМетод(ДанныеЖелудя.Желудь, "ИмяИнструмента") Тогда
		...
		Метод.ТелоМетода(СтрШаблон("Возврат %1;", СтроковыйЛитерал(Имя)));
	...
	Если Не ЕстьМетод(ДанныеЖелудя.Желудь, "ОписаниеИнструмента") Тогда
		...
		Метод.ТелоМетода(СтрШаблон("Возврат %1;", СтроковыйЛитерал(Описание)));
		ПостроительДекоратора.Метод(Метод);
	КонецЕсли;
...
Процедура ПодготовитьСхемаИнструмента(ПостроительДекоратора, ДанныеЖелудя)
	...
	Для Каждого Свойство из Свойства Цикл
		...
		Если Аннотация = Неопределено Тогда
			// Свойство без аннотации - не параметр инструмента, например
			// зависимость, внедренная конструктором желудя
			Продолжить;
		КонецЕсли;
		Имя = РаботаСАннотациями.ПолучитьЗначениеПараметраАннотации(Аннотация, "Имя",
		...
		ТелоМетода.Добавить(СтрШаблон("Схема.Вставить(%1, ТекущийЭлемент);", СтроковыйЛитерал(Имя)));
		ТелоМетода.Добавить(СтрШаблон("ТекущийЭлемент.Вставить(""description"", %1);", СтроковыйЛитерал(Описание)));
		...
		КонецЕсли;
		ТелоМетода.Добавить(СтрШаблон("ТекущийЭлемент.Вставить(""type"", %1);", СтроковыйЛитерал(Тип)));
		Если ЗначениеЗаполнено(Формат) Тогда
			ТелоМетода.Добавить(СтрШаблон("ТекущийЭлемент.Вставить(""format"", %1);", СтроковыйЛитерал(Формат)));
		КонецЕсли;
		Если Обязательный Тогда
			ТелоМетода.Добавить(СтрШаблон("ОбязательныеПоля.Добавить(%1);", СтроковыйЛитерал(Имя)));
		КонецЕсли;
	КонецЦикла;
...
Процедура ПодготовитьВыполнитьИнструмент(ПостроительДекоратора, ДанныеЖелудя)
	...
	Для Каждого Свойство из Свойства Цикл
		Аннотация = РаботаСАннотациями.ПолучитьАннотацию(Свойство, "ПараметрИнструмента");
		Если Аннотация = Неопределено Тогда
			// Свойство без аннотации - не параметр инструмента, его значение
			// (например, зависимость желудя) не перезаписывается при вызовах
			Продолжить;
		КонецЕсли;
		Имя = РаботаСАннотациями.ПолучитьЗначениеПараметраАннотации(Аннотация, "Имя",
			СтроковыеФункции.СтрокаЛатиницей(Свойство.Имя));
...
// Превращает строковое значение в литерал генерируемого кода OneScript.
// ЗаписатьJSON для этого не подходит: экранирование JSON (\n, \", \\)
// не интерпретируется в строковых литералах OneScript, поэтому спецсимволы
// попадали клиенту как есть, а кавычка в значении ломала компиляцию декоратора
Функция СтроковыйЛитерал(Знач Значение)
