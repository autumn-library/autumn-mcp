...
Перем _Прогресс;   // MCP_Прогресс - репортёр notifications/progress (задаёт фабрика MCP_Дуб)
Перем Рефлектор;
Перем _JSON;       // MCP_JSON - сериализация встроенными средствами OneScript
...
Процедура Запустить() Экспорт
	...
	Пока Истина Цикл
		...
		Если ТекстКоманды = Неопределено Тогда
			// stdin закрыт (клиент отключился) — выходим из цикла. Иначе ПрочитатьСтроку
			// на EOF возвращает Неопределено мгновенно и цикл крутит CPU на 100%, а
		...
		Если Не ЗначениеЗаполнено(ТекстКоманды) Тогда
			Продолжить;
		КонецЕсли;
		Команда = _JSON.ВСоответствие(ТекстКоманды);
		Если ТипЗнч(Команда) <> Тип("Соответствие") Тогда
			// JSON-RPC запрос обязан быть объектом. Корень-массив (в т.ч. батч),
			// null, число или строка - валидный JSON, но не объект: пропускаем
			// строку, иначе Команда.Получить("method") упадёт с ошибкой типа и
			// уронит весь цикл обслуживания (как и неизвестные методы - молча).
			Продолжить;
		КонецЕсли;
		...
		Если Метод = "initialize" Тогда
			...
			Ответ = _JSON.ВСтроку(Соответствие);
		...
		ИначеЕсли Метод = "tools/list" Тогда
			...
			Для каждого КлючИЗначениеИнструмента Из Инструменты Цикл
				...
				// Опциональный блок annotations (title / *Hint протокола MCP). Инструмент
				// может реализовать АннотацииИнструмента() -> Соответствие|Структура с
				// заполненными ключами. Инструменты без метода блок не эмитят
				// (обратная совместимость с декларативными инструментами).
				Если Рефлектор.МетодСуществует(MCP_Инструмент, "АннотацииИнструмента") Тогда
					БлокАннотаций = MCP_Инструмент.АннотацииИнструмента();
					Если БлокАннотаций <> Неопределено И БлокАннотаций.Количество() > 0 Тогда
						Инструмент.Вставить("annotations", БлокАннотаций);
					КонецЕсли;
				КонецЕсли;
		...
		ИначеЕсли Метод = "tools/call" Тогда
			...
			ЭтоОшибка = Ложь;
			// progressToken (если клиент его передал) включает notifications/progress
			// на время вызова: длительная синхронная команда шлёт прогресс через общий
			// MCP_Прогресс, а клиент по нему сбрасывает таймаут вызова.
			НачатьПрогресс(ИзвлечьТокенПрогресса(ПараметрыКоманды));
			Попытка
			...
			Если ТипРезультата <> Тип("Строка") И ТипРезультата <> Тип("Дата") Тогда
				РезультатВыполнения = _JSON.ВСтроку(РезультатВыполнения);
			КонецЕсли;
...
// progressToken из params._meta (Неопределено — клиент прогресс не запрашивал).
Функция ИзвлечьТокенПрогресса(ПараметрыКоманды)
	Если ПараметрыКоманды = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	Мета = ПараметрыКоманды.Получить("_meta");
	Если Мета = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	Возврат Мета.Получить("progressToken");
КонецФункции

Процедура НачатьПрогресс(Знач Токен)
	Если _Прогресс <> Неопределено Тогда
		_Прогресс.Начать(Токен);
	КонецЕсли;
КонецПроцедуры

Процедура ЗавершитьПрогресс()
	Если _Прогресс <> Неопределено Тогда
		_Прогресс.Завершить();
	КонецЕсли;
КонецПроцедуры
