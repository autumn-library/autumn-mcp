	...
	Пока Истина Цикл
		...
		ИначеЕсли Метод = "tools/list" Тогда
			...
			Для каждого КлючИЗначениеИнструмента Из Инструменты Цикл
				...
				// заполненными ключами. Инструменты без метода попадают в ветку Исключение
				// и блок не эмитят (обратная совместимость с декларативными инструментами).
				Попытка
					БлокАннотаций = MCP_Инструмент.АннотацииИнструмента();
				Исключение
					БлокАннотаций = Неопределено;
				КонецПопытки;
